...
if dataset == 'SingleElectron':
    ...
    IN_DIR   = '/afs/cern.ch/work/c/csutanta/public/unskimmed_histograms/v2_scalefactor/{year}/unskimmed_SingleMuon_'
    OUT_DIR  = f'{outdir}/SingleMuon/'
else:
    print('need to provide dataset! exiting.')
    exit()
...
def fill_pass_fail(h_in, h_out, cuts):
    ...
    for iCut in range(1, len(cutsX)):
        ...
        # print(f'{h_in.GetName()=}: {h_in.Integral()}')
        # print(f'{h_out.GetName()=} : {h_out.Integral()}')
        ...
        for sel in SELS:
            ...
            for tag in TAGGERS.keys():
                ...
                for year in YEARS:
                    ...
                    ERAS = YEARS[year]
                    for era in ERAS:   ## i think we can nest the year on top of era, so that it reads eeach year too. We will need to change the h_out name to not include year. callit runII
                        if VERBOSE: print('\nNow looking at era %s' % era)
                        h_in_name = 'evt/'+DATA+'_'+era+'/'+tag+'_sel_'+sel+'_noweight'
                        if year == '2018':
                            if DATA == 'SingleElectron':
                                h_in_name = 'evt/EGamma_'+era+'/'+tag+'_sel_'+sel+'_noweight'
                        ...
                        h_out_name = DATA+'_runII_'+cat+'_'+TAGNM[tag]
                        ...
                        #print(f'{h_in.GetName()=} : {h_in.Integral()}')
                        #print(f'{h_outs[sel][h_out_name].GetName()=} : {h_outs[sel][h_out_name].Integral()}')
                ...
                for mc in MC_2bq+MC_2b+MC_bqq+MC_1b+MC_0b:
                    ## do for year in YEARS in this level, but not including "if mc in MC_2bq: systs = ['2bq','2B2Q']" This fills all the h_out before filling h_MC. should fix double adding h_MC
                    for year in YEARS:
                        in_file_str = IN_DIR.format(year=year)+cat+'.root'
                        in_file = R.TFile(in_file_str, 'open')
                        ...
                        if not h_out_name in h_outs[sel].keys():
                            # print(f'{h_out_name}') ## this loop only runs when new bdtHi/Med/Lo/Veto is being run
                            h_outs[sel][h_out_name] = R.TH1D(h_out_name, h_out_name, nCuts+1, 0, nCuts+1)
                            h_outs[sel][h_out_name].SetDirectory(0) ## Save locally
                        #else:
                        #    print('\n\nHw is %s already in h_outs[%s]??? Quitting.' % (h_out_name, sel))
                        #    sys.exit()
                        fill_pass_fail(h_in, h_outs[sel][h_out_name], TAGGERS[tag])
